...
for step in range(1000):
    ...
    '''#What I first wrote about feedforward code from X matrix to h matrix:
    ...
    '''#What I first wrote about feedforward code from h matrix to K matrix: 
    for j2 in range (2):
        #for l in range (N):
            #print("h:"+str(h[l][0]))
    ...
    all_loss[step, 0] = cross_entropy_error(S,t)


    db1 = np.mean((S - t) * K * (1 - K),axis = 0) #axis= 0 is an average of columns. If axis = 1, average in rows.
    print("db1-->")
    print(db1)

    sum = 0
    for i in range (len(h)):
        sum += np.kron(h.T[:, i], db1).reshape((3, 2)) #np.kron is the tensor product
    dW1 = sum / len(h)
    print("dW1-->")
    print(dW1)
    ...
    for j in range(len(X)):
        sum2 += np.kron(X.T[:, j], db2).reshape((2,3))
    dW2 = sum2 / len(X)
    print("dW2-->")
    print(dW2)
